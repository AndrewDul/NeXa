...
| `m2_6a_gemini_live_probe.py` | The M2.6A real-hardware probe. `--dry` = build the cloud-side object graph only (no device, no network); no flag = full session (mic → XVF3800 AEC path → Gemini Live → speaker) with a one-clock event timeline, Pipecat-#5465 NOT_READY audio accounting, AEC-reference health, and a token/cost estimate → timestamped JSON in this directory. Hard 15-minute session cap. |
