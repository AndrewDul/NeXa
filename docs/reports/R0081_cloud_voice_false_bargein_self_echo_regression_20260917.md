...
**Exactly one operator procedure**: a fresh paired run, run → 30 seconds of silence → run, executed back to back in one session.
...
The existing run-id mechanism (§16.12) already guarantees the two WAV pairs stay separate — no script change needed. **Return the full output of BOTH fresh runs** (per-cycle tables, run ids, `quiet_before_rms`, first-half/second-half means, min/max). Interpret per §16.15's persistence/reset/irregular framework — this does **not** supersede or discard §16.14's historical `20260918T074722Z` result, which remains the first documented evidence of large in-stream non-monotonic residual variation. This does not use Gemini, VAD, or touch production code/DSP registers/Silero/gain.
